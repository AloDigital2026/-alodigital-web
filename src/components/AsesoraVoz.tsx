...
export const AsesoraVoz: React.FC<Props> = ({ onCerrar }) => {
  ...
  const reproducirFragmento = (base64Audio: string) => {
    if (!playbackContextRef.current) {
      ...
      colaReproduccionRef.current = playbackContextRef.current.currentTime;
    }
    ...
    const inicio = Math.max(colaReproduccionRef.current, ctx.currentTime);
    fuente.start(inicio);
    colaReproduccionRef.current = inicio + audioBuffer.duration;
